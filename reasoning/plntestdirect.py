"""The Python twin of examples/reasoning/plntestdirect.metta.
...
def twin(m):
    ...
    m += expr(
        S["="],
        expr(S["sentence"], expr(S["Inheritance"], S["a"], S["b"]), expr(S["stv"], 0.9, 0.9)),
        expr(S["once"], val(value=True)),
    )

    # (= (sentence (Inheritance b c) (stv 0.9 0.9)) (once True))
    m += expr(
        S["="],
        expr(S["sentence"], expr(S["Inheritance"], S["b"], S["c"]), expr(S["stv"], 0.9, 0.9)),
        expr(S["once"], val(value=True)),
    )
    ...
    m += expr(
        S["="],
        expr(S["sentence"], expr(S["Inheritance"], V["A"], V["C"]), V["TV"]),
        expr(
            S["once"],
            expr(
                S["and"],
                expr(
                    S["and"],
                    expr(S["sentence"], expr(S["Inheritance"], V["A"], V["B"]), V["T1"]),
                    expr(S["sentence"], expr(S["Inheritance"], V["B"], V["C"]), V["T2"]),
                ),
                expr(
                    S["="],
                    V["TV"],
    ...
    yield m.eval(
        expr(
            ...
            expr(
                S["let"],
                V["derivation"],
                expr(S["sentence"], expr(S["Inheritance"], S["a"], S["c"]), V["TV"]),
                V["TV"],
            ),
            expr(S["stv"], 0.8166666666666668, 0.9),
        )
    )
