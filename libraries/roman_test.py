"""examples/libraries/roman_test.metta in Python: lib_roman, walked end to end.
...
def twin(m):
    ...
    assert m.fn("map-nested")(S["+"](1), (1, (2, 3))) == expr(2, expr(3, 4))
    assert m.fn("fold-flat")(S["+"], 0, (1, 2, 3)) == 6
    assert m.fn("foldr-flat")(S.cons, (), (1, (2, 3), 4)) == expr(1, expr(2, 3), 4)
    ...
    assert alpha_eq(m.fn("/=a\\")((1, 2, V.a), (2, V.a, 4)), expr(2, V.a))
    ...
    assert alpha_eq(m.fn("\\=a/")((1, 2, V.a), (2, V.a, 4)), expr(1, 2, V.a, 4))
    ...
    assert m.one(S.let(S["@"](V.lst, S.cons(V.h, V.t)), (1, 2, 3), expr(V.lst, V.h, V.t))) == expr(expr(1, 2, 3), 1, expr(2, 3))  # rung: reverse function matching: `let` unifies a CALL against a value and runs the function backwards, which Python's assignment cannot do
    ...
    assert m.one(S.let(S.tail(V.xs), (1, 2, 3), V.xs)) == expr(2, 3)  # rung: reverse function matching: `let` unifies a CALL against a value and runs the function backwards, which Python's assignment cannot do
    ...
    assert m.one(S.let(S.init(V.xs), (1, 2, 3), V.xs)) == expr(1, 2)  # rung: reverse function matching: `let` unifies a CALL against a value and runs the function backwards, which Python's assignment cannot do
    assert m.one(S.let(S.rcons(V.xs, V.x), (1, 2, 3), expr(V.xs, V.x))) == expr(expr(1, 2), 3)  # rung: reverse function matching: `let` unifies a CALL against a value and runs the function backwards, which Python's assignment cannot do
