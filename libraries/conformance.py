...
twin names it. The space it checks is handed over as the HANDLE it is.
...
Two names keep the bracket because their MeTTa spelling really has
underscores: `lib_conformance` and `import_prolog_functions_from_file`. The
attribute door maps every underscore to a hyphen, so `S.lib_conformance` is the
