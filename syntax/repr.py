"""examples/syntax/repr.metta in Python: how an atom prints.
...
The expected texts cross through `val`, because that is what they are in the
original too: a `"..."` in MeTTa source is a String atom, not program text.
...
PRINTED = [
    (val(42), val("42")),
    (val("42"), val('"42"')),
    (S.A(S.B(S.C)), val("(A (B C))")),
    (S.A((S[","], S.B, S[","], S.C, S[","])), val("(A (, B , C ,))")),
    (S["2025_12_12"], val("2025_12_12")),
    ((), val("()")),
]
