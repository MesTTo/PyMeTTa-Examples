"""The Python twin of examples/spaces/matespace.metta.
...
def twin(m):
    ...
    m += expr(
        ...
        expr(
            ...
            S["done"],
            expr(
                S["let"], V["temp1"], expr(S["expand"]), expr(S["expandK"], expr(S["-"], V["n"], 1))
            ),
        ),
    )
    ...
    yield m.eval(
        expr(
            S["test"],
            expr(S["length"], expr(S["collapse"], expr(S["mate-space-demo"], 390))),
            1063919,
        )
    )
