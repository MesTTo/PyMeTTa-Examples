...
Both equations are written at the container door. `match-types` is hyphenated,
which `name=` handles, but `match-type-or`'s body CALLS it, and a compiled body
resolves a free name EXACTLY, so a hyphenated function cannot be reached from
one (wave one recorded that against P14.4 for `fibsmart`). Writing one of the
pair at each door would say the two equations differ in kind, and they do not.
...
def twin(m):
    ...
    yield m.eval(
        S.test(
            S["match-type-or"](TRUE, S.Number, S.Number), TRUE
        )
    )
    # !(test (match-type-or False Number Number) True)
    yield m.eval(
        S.test(
            S["match-type-or"](FALSE, S.Number, S.Number), TRUE
        )
    )
    # !(test (match-type-or True Number Bool) True)
    yield m.eval(
        S.test(
            S["match-type-or"](TRUE, S.Number, S.Bool), TRUE
        )
    )
    # !(test (match-type-or False Number Bool) False)
    yield m.eval(
        S.test(
            S["match-type-or"](FALSE, S.Number, S.Bool), FALSE
        )
    )
