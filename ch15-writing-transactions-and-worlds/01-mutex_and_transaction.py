...
#: RE-OBSERVED 2026-09-19 under 'full-lane/294/workers=32', 23345..23357 over
#: 25 under 'full-lane/294/workers=32' to 23258..23264 over 10: the trunk
...
#: RE-OBSERVED 2026-09-19 under 'full-lane/294/workers=32', 23258..23264 over
#: 10 under 'full-lane/294/workers=32' to 23258..23265 over 10: cost follows
...
#: ENVELOPED 2026-09-19: the envelope 23258..23265 over 10 becomes the envelope
#: 23258..23267 over 14: under the lane's own protocol (full-
#: lane/294/workers=32, ten rounds) the counter reads 23258 every time, and
#: under the gate's concurrent lanes and alone it reads 23259, 23261, 23264,
#: 23267, each reading exact on its run; the extra mode is the same program's
#: work done on a different thread under load, a loader flight or a settle step
#: the foreground runs itself when the worker is late, which the join
#: accounting does not reach; an envelope states what was observed and the
...
#: RE-OBSERVED 2026-09-21 under 'full-lane/323/workers=32/file-search-cache-
#: time=9223372036854775807/before-boot', 23661..23666 to 23657..23669 over 20
#: observations: ten observations did not cover the tail of an example whose
...
#: time=9223372036854775807/before-boot', 23657..23669 to 23638..23647 over 20
#: observations: the tree under these envelopes moved after their last
...
#: time=9223372036854775807/before-boot', 23638..23647 over 20 to 23638..23650
#: over 21: the whole-lane run on HEAD 26de4ddcf with the twins at a8487162,
#: its engine diff empty and its battery's governed QLF set purged so that
#: every artifact was compiled where it sits, read 23650, 3 above the twenty
#: rounds' maximum; the commits after the observed 6a7ac233d touch no path this
#: twin runs; a whole-lane run under this protocol on this runtime is an
#: observation, so the envelope is the union of the extrema and the sum of the
#: counts. docs/journal/2026-09-18-schedule-independent-counters.md places the
#: spread the envelope twins keep in schedule-bound work the join accounting
#: does not reach; which path adds these three inferences is not isolated
#: [measured 2026-09-24: 1 whole-lane run, sh extensions/python/check.sh twins
#: in battery 5 after metta_qlf_boot:purge_all_qlf; command=python
#: extensions/python/tools/twin_coverage.py; commit=WORKTREE].
#: POOLED 2026-09-24 under 'full-lane/323/workers=32/file-search-cache-
#: time=9223372036854775807/before-boot', 23638..23650 over 21 to 23638..23650
#: over 28: the 7 whole-lane runs of this runtime the envelope did not yet hold
#: read the pin run's first lane 23639; the pin run's second lane 23641; the
#: pin run's third lane 23643; the first final lane 23638; the purged lane on
#: 49e2b250d 23640; the purged lane on 26de4ddcf 23645; the purged lane after
#: 878b8bd9 23640. They are the pin run's own lanes on the 6a7ac233d snapshot
...
#: of the counts [measured 2026-09-24: 7 whole-lane runs, sh
#: extensions/python/check.sh twins in battery 5; command=python
#: extensions/python/tools/twin_coverage.py; commit=WORKTREE].
#: POOLED 2026-09-24 under 'full-lane/323/workers=32/file-search-cache-
#: time=9223372036854775807/before-boot', 23638..23650 over 28 to 23638..23652
#: over 48: the twenty full-lane rounds in battery 97 on the tree 0a81c782f
#: made read 23638..23652; that commit changes how a library's Prolog half
#: loads, and this twin's window imports no library, so the tree did not move
#: under it (its twin read 23638 in the lanes at 0a81c782f and at its parent),
#: and the two above its maximum are the schedule-bound spread the 2026-09-18
#: journal places outside the join accounting; the about 10.3k per Prolog-half
...
#: every governed half a half loads compiled from source in every process; a
#: whole-lane run under this protocol on this twin's unchanged runtime is an
#: observation, so the envelope is the union of the extrema and the sum of the
#: counts [measured 2026-09-24: 20 full-lane observations; command=python
#: extensions/python/tools/twin_coverage.py --observe --rounds 20;
#: commit=0a81c782fd6ba00984c36e58e228f73bca810dee].
#: RE-OBSERVED 2026-09-24 under 'full-lane/323/workers=32/file-search-cache-
#: time=9223372036854775807/before-boot', 23638..23652 to 23658..23670 over 21
#: observations: the tree moved under this envelope after its observation at
#: 0a81c782f: read serially, the minimum of seven fresh processes a side, on
#: eighteen first-parent rungs from 0a81c782f to the fixed tree 4ff69551e in
#: one battery, its twin holds 23,644 to 23,646 from 0a81c782f through
#: ae1cc8936, 23,650 to 23,659 from 850d2a660 through d4a365c16, and 23,666 or
#: 23,667 from d781eab8f on: provider-carry's 850d2a660, which reads a native
#: space's lengths in ascending order, moves it +11 on its rung, and gate-
#: perf's d781eab8f, which copies a withdrawn equation once, +9 on its rung,
#: where its example moves +13 as well; the rungs whose commits change nothing
#: it runs (3717f6894, f8496a9f1, 5d78325cf and 4ff69551e) move a minimum of
#: seven by up to 9 either way, so no smaller step is placed, and the band's
#: midpoint moved +19; an empirical envelope is a claim about one scheduler's
